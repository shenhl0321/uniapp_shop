<template>
	<view>
		...
		<view class="main-menu">
			<view class="menu-item" :class="{'active': menuIndex == 1}" @click='handleMenuSelect(1)'>
				未使用
			</view>
			<view class="menu-item" :class="{'active': menuIndex == 2}" @click='handleMenuSelect(2)'>
				已使用
			</view>
			<view class="menu-item" :class="{'active': menuIndex == 3}" @click='handleMenuSelect(3)'>
				已过期
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			loadData() {
				this.$request.get('/api/member/getUnUsedCoupon', ).then((res) => {
					this.couponList = res
				}, (rej) => {
					console.log(rej)
				})
